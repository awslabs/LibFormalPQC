...
is
   ...
   --  Unconstrained arrays of U32 and U64 words
   type U16_Seq is array (N32 range <>) of U16;
   type U32_Seq is array (N32 range <>) of U32;
   type U64_Seq is array (N32 range <>) of U64;

   package Param
   is
      subtype FFT_Power is N32 range 1 .. 5;

#if HQC_PARAM = 1 then

      --  ====================
      --  Parameters for HQC-1
      --  ====================
      CRYPTO_ALGNAME         : constant String := "HQC-1";
      CRYPTO_SECRETKEYBYTES  : constant := 2321;
      CRYPTO_PUBLICKEYBYTES  : constant := 2241;
      CRYPTO_BYTES           : constant := 32;
      CRYPTO_CIPHERTEXTBYTES : constant := 4433;

      N              : constant := 17669; -- parameter n of the scheme
      N_MU           : constant := 243079; -- floor(2**32 / N) for Barrett Reduction
      N1             : constant := 46;    -- (bytes) parameter n1 of the scheme (length of Reed–Solomon code)
      N2             : constant := 384;   -- parameter n2 of the scheme (length of Duplicated Reed–Muller code)
      N1N2           : constant := 17664; -- length in bits of the concatenated code
      OMEGA          : constant := 66;    -- parameter omega of the scheme
      OMEGA_E        : constant := 75;    -- parameter omega_e of the scheme
      OMEGA_R        : constant := 75;    -- parameter omega_r of the scheme
      SECURITY       : constant := 128;   -- security level corresponding to the chosen parameters
      SECURITY_BYTES : constant := 16;    -- security level in bytes
      DFR_EXP        : constant := 128;   -- decryption failure rate exponent
      PDELTA         : constant := 15;    -- Define the error-correcting capacity (delta) of the Reed–Solomon code
      K              : constant := 16;          -- Define the size of the information bits of the Reed–Solomon code
      G              : constant := 31;          -- Define the size of the generator polynomial of the Reed–Solomon code
      FFT            : constant FFT_Power := 4; -- Exponent for additive FFT (2^PARAM_FFT points)
      UTILS_REJECTION_THRESHOLD : constant := 16_767_881; -- Rejection threshold for uniform sampling in [0, PARAM_N)

      --  RMMF = "Reed Muller Multiplicity Factor"
      --  How many times each codeword is repeated = N2/128
      pragma Assert (N2 mod 128 = 0);
      RMMF : constant := N2 / 128;
      pragma Assert (RMMF = 3);

      VEC_N_SIZE_BYTES    : constant := 2209;
      VEC_N1N2_SIZE_BYTES : constant := 2208;
      VEC_N_SIZE_64       : constant := 277;
      --  N1 bytes (46) is 368 bits, which can be stored in 6 64-bit words
      --  with 16 bits of unused padding
      VEC_N1_SIZE_64      : constant := 6;
      VEC_N1N2_SIZE_64    : constant := 276;
      VEC_K_SIZE_64       : constant := 2;

#elsif HQC_PARAM = 3 then

      --  ====================
      --  Parameters for HQC-3
      --  ====================
      CRYPTO_ALGNAME         : constant String := "HQC-3";
      CRYPTO_SECRETKEYBYTES  : constant := 4602;
      CRYPTO_PUBLICKEYBYTES  : constant := 4514;
      CRYPTO_BYTES           : constant := 32;
      CRYPTO_CIPHERTEXTBYTES : constant := 8978;

      N              : constant := 35851;  -- parameter n of the scheme
      N_MU           : constant := 119800; -- floor(2**32 / N) for Barrett Reduction
      N1             : constant := 56;    -- (bytes) parameter n1 of the scheme (length of Reed–Solomon code)
      N2             : constant := 640;   -- parameter n2 of the scheme (length of Duplicated Reed–Muller code)
      N1N2           : constant := 35840; -- length in bits of the concatenated code
      OMEGA          : constant := 100;   -- parameter omega of the scheme
      OMEGA_E        : constant := 114;   -- parameter omega_e of the scheme
      OMEGA_R        : constant := 114;   -- parameter omega_r of the scheme
      SECURITY       : constant := 192;   -- security level corresponding to the chosen parameters
      SECURITY_BYTES : constant := 24;    -- security level in bytes
      DFR_EXP        : constant := 192;   -- decryption failure rate exponent
      PDELTA         : constant := 16;    -- Define the error-correcting capacity (delta) of the Reed–Solomon code
      K              : constant := 24;          -- Define the size of the information bits of the Reed–Solomon code
      G              : constant := 33;          -- Define the size of the generator polynomial of the Reed–Solomon code
      FFT            : constant FFT_Power := 5; -- Exponent for additive FFT (2^PARAM_FFT points)
      UTILS_REJECTION_THRESHOLD : constant := 16_742_417; -- Rejection threshold for uniform sampling in [0, PARAM_N)

      --  RMMF = "Reed Muller Multiplicity Factor"
      --  How many times each codeword is repeated = N2/128
      pragma Assert (N2 mod 128 = 0);
      RMMF : constant := N2 / 128;
      pragma Assert (RMMF = 5);

      VEC_N_SIZE_BYTES    : constant := 4482;
      VEC_N1N2_SIZE_BYTES : constant := 4480;
      VEC_N_SIZE_64       : constant := 561;
      VEC_N1_SIZE_64      : constant := 7;
      VEC_N1N2_SIZE_64    : constant := 560;
      VEC_K_SIZE_64       : constant := 3;

#elsif HQC_PARAM = 5 then

      --  ====================
      --  Parameters for HQC-5
      --  ====================
      CRYPTO_ALGNAME         : constant String := "HQC-5";
      CRYPTO_SECRETKEYBYTES  : constant := 7333;
      CRYPTO_PUBLICKEYBYTES  : constant := 7237;
      CRYPTO_BYTES           : constant := 32;
      CRYPTO_CIPHERTEXTBYTES : constant := 14421;

      N              : constant := 57637;  -- parameter n of the scheme

      N_MU           : constant := 74517; -- floor(2**32 / N) for Barrett Reduction

      N1             : constant := 90;    -- (bytes) parameter n1 of the scheme (length of Reed–Solomon code)
      N2             : constant := 640;   -- parameter n2 of the scheme (length of Duplicated Reed–Muller code)
      N1N2           : constant := 57600; -- length in bits of the concatenated code
      OMEGA          : constant := 131;   -- parameter omega of the scheme
      OMEGA_E        : constant := 149;   -- parameter omega_e of the scheme
      OMEGA_R        : constant := 149;   -- parameter omega_r of the scheme
      SECURITY       : constant := 256;   -- security level corresponding to the chosen parameters
      SECURITY_BYTES : constant := 32;    -- security level in bytes
      DFR_EXP        : constant := 256;   -- decryption failure rate exponent
      PDELTA         : constant := 29;    -- Define the error-correcting capacity (delta) of the Reed–Solomon code
      K              : constant := 32;          -- Define the size of the information bits of the Reed–Solomon code
      G              : constant := 59;          -- Define the size of the generator polynomial of the Reed–Solomon code
      FFT            : constant FFT_Power := 5; -- Exponent for additive FFT (2^PARAM_FFT points)
      UTILS_REJECTION_THRESHOLD : constant := 16_772_367; -- Rejection threshold for uniform sampling in [0, PARAM_N)
      ...
      VEC_N_SIZE_BYTES    : constant := 7205;
      VEC_N1N2_SIZE_BYTES : constant := 7200;
      VEC_N_SIZE_64       : constant := 901;
      VEC_N1_SIZE_64      : constant := 12;
      VEC_N1N2_SIZE_64    : constant := 900;
      VEC_K_SIZE_64       : constant := 4;
#end if;

      function Ceil_Divide (A : in Natural; B : in Positive) return Natural
        is ((A / B) + (if (A mod B) = 0 then 0 else 1));

      --  Sanity checks on derived constants
      pragma Assert (VEC_N_SIZE_BYTES = Ceil_Divide (N, 8));
      pragma Assert (VEC_N1N2_SIZE_BYTES = Ceil_Divide (N1N2, 8));
      pragma Assert (VEC_N_SIZE_64 = Ceil_Divide (N, 64));
      pragma Assert (VEC_N1_SIZE_64 = Ceil_Divide (N1, 8));
      pragma Assert (VEC_K_SIZE_64 = Ceil_Divide (K, 8));
      pragma Assert (VEC_N1N2_SIZE_64 = Ceil_Divide (N1N2, 64));

      --  Type for the Reed-Solomon Poly Coefficients and Alpha Tables, the dimensions and values of which
      --  depend on the parameter set
      subtype RS_G_Index is N32 range 0 .. G - 1;
      subtype RS_G_Table is Byte_Seq (RS_G_Index);

      subtype RS_Row_Index    is N32 range 0 .. 2 * Param.PDELTA - 1;
      subtype RS_Column_Index is N32 range 0 .. Param.N1 - 2;

      subtype RS_Alpha_Column is U16_Seq (RS_Column_Index);
      type RS_Alpha_Table is array (RS_Row_Index) of RS_Alpha_Column;

#if HQC_PARAM = 1 then

      RS_Poly_Coeffs : constant RS_G_Table :=
        (89, 69, 153, 116, 176, 117, 111, 75, 73, 233, 242, 233, 65, 210, 21, 139, 103, 173, 67, 118, 105, 210, 174, 110,
         74, 69, 228, 82, 255, 181, 1);

      Alpha_IJ_Pow : constant RS_Alpha_Table :=
      ...
      RS_Poly_Coeffs : constant RS_G_Table :=
        (49, 167, 49, 39, 200, 121, 124, 91, 240, 63, 148, 71, 150, 123, 87, 101,
         32, 215, 159, 71, 201, 115, 97, 210, 186, 183, 141, 217, 123, 12, 31,
         243, 180, 219, 152, 239, 99, 141, 4, 246, 191, 144, 8, 232, 47, 27, 141,
         178, 130, 64, 124, 47, 39, 188, 216, 48, 199, 187, 1);
      ...
      --  Common Parameters
      --  =================
      SHARED_SECRET_BYTES : constant := CRYPTO_BYTES;

      SEED_BYTES : constant := 32;
      SALT_BYTES : constant := 16;

      --  for all param sets
      N1N2_Min       : constant := 17664;
      N1N2_Max       : constant := 57600;
      OMEGA_MIN      : constant := 66;
      OMEGA_MAX      : constant := 149;

      M            : constant := 8;   -- Define the degree m of the Galois field GF(2^m)
      GF_MUL_ORDER : constant := 255; -- Size of the multiplicative group of GF(2^PARAM_M) (2^PARAM_M−1)

      M_Mask       : constant := 2**M - 1;
      GF_POLY      : constant := 16#11D#;     -- Generator polynomial of GF(2^PARAM_M) in hexadecimal form

      VEC_K_SIZE_BYTES  : constant := K;
      VEC_N1_SIZE_BYTES : constant := N1;

      --  Sanity checks
      pragma Assert (N1N2_Min <= N1N2);
      pragma Assert (N1N2_Max >= N1N2);

      pragma Assert (OMEGA_R = OMEGA_E);
      pragma Assert (OMEGA < OMEGA_E);
      pragma Assert (OMEGA < OMEGA_R);
      pragma Assert (OMEGA_MIN <= OMEGA);
      pragma Assert (OMEGA_MAX >= OMEGA_R);

      pragma Assert (CRYPTO_PUBLICKEYBYTES = VEC_N_SIZE_BYTES + SEED_BYTES);
      pragma Assert (CRYPTO_CIPHERTEXTBYTES = VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES + SALT_BYTES);
      pragma Assert (SECURITY_BYTES = K);
   end Param;


   --  Constrained (fixed-length) arrays of bytes and words
   ...
   subtype Seed_Index  is I32 range 0 .. Param.SEED_BYTES - 1;
   subtype Seed_Bytes  is Byte_Seq (Seed_Index);

   subtype Security_Index  is I32 range 0 .. Param.SECURITY_BYTES - 1;
   subtype Security_Bytes  is Byte_Seq (Security_Index);

   subtype Salt_Index  is I32 range 0 .. Param.SALT_BYTES - 1;
   subtype Salt_Bytes  is Byte_Seq (Salt_Index);

   subtype Ciphertext_Index is I32 range 0 .. Param.CRYPTO_CIPHERTEXTBYTES - 1;
   subtype Ciphertext is Byte_Seq (Ciphertext_Index);

   subtype Encapsulation_Key_Index is I32 range 0 .. Param.CRYPTO_PUBLICKEYBYTES - 1;
   subtype Encapsulation_Key is Byte_Seq (Encapsulation_Key_Index);

   subtype Decapsulation_Key_Index is I32 range 0 .. Param.CRYPTO_SECRETKEYBYTES - 1;
   subtype Decapsulation_Key is Byte_Seq (Decapsulation_Key_Index);

   type HQC_Keypair is record
      EK : Encapsulation_Key;
      DK : Decapsulation_Key;
   end record;

   subtype Shared_Secret_Index is N32 range 0 .. Param.SHARED_SECRET_BYTES - 1;
   subtype Shared_Secret is Byte_Seq (Shared_Secret_Index);

   subtype Index_N_Bits_As_Words    is N32 range 0 .. Param.VEC_N_SIZE_64 - 1;
   subtype Index_2N_Bits_As_Words   is N32 range 0 .. 2 * Param.VEC_N_SIZE_64 - 1;
   subtype Index_N1N2_Bits_As_Words is N32 range 0 .. Param.VEC_N1N2_SIZE_64 - 1;


   subtype U64_Seq_N_Bits_As_Words    is U64_Seq (Index_N_Bits_As_Words);
   subtype U64_Seq_2N_Bits_As_Words   is U64_Seq (Index_2N_Bits_As_Words);
   subtype U64_Seq_N1N2_Bits_As_Words is U64_Seq (Index_N1N2_Bits_As_Words);

   subtype Index_N_Bits_As_Bytes_Padded is I32 range 0 .. (Param.VEC_N_SIZE_64 * 8) - 1;
   subtype Seq_N_Bits_As_Bytes_Padded is Byte_Seq (Index_N_Bits_As_Bytes_Padded);

   N_Padding_Count : constant := (Param.VEC_N_SIZE_64 * 8) - Param.VEC_N_SIZE_BYTES;
   subtype N_Padding_Index is N32 range 0 .. N_Padding_Count - 1;
   subtype N_Padding_Block is Byte_Seq (N_Padding_Index);
   N_Padding : constant N_Padding_Block := (others => 0);

   N1N2_Padding_Count : constant := (Param.VEC_N_SIZE_64 * 8) - Param.VEC_N1N2_SIZE_BYTES;
   pragma Assert (N1N2_Padding_Count = 8); --  for HQC-1
   subtype N1N2_Padding_Index is N32 range 0 .. N1N2_Padding_Count - 1;
   subtype N1N2_Padding_Block is Byte_Seq (N1N2_Padding_Index);
   N1N2_Padding : constant N1N2_Padding_Block := (others => 0);

   subtype Index_N1N2_Bits_As_Bytes_Padded is I32 range 0 .. (Param.VEC_N1N2_SIZE_64 * 8) - 1;
   subtype Seq_N1N2_Bits_As_Bytes_Padded is Byte_Seq (Index_N1N2_Bits_As_Bytes_Padded);

   subtype Index_N_Bits_As_Bytes is I32 range 0 .. Param.VEC_N_SIZE_BYTES - 1;
   subtype Seq_N_Bits_As_Bytes is Byte_Seq (Index_N_Bits_As_Bytes);
   pragma Assert (Seq_N_Bits_As_Bytes'Length = Param.VEC_N_SIZE_BYTES); --  for HQC-1

   subtype Index_N1N2_Bits_As_Bytes is I32 range 0 .. Param.VEC_N1N2_SIZE_BYTES - 1;
   subtype Seq_N1N2_Bits_As_Bytes is Byte_Seq (Index_N1N2_Bits_As_Bytes);
   pragma Assert (Seq_N1N2_Bits_As_Bytes'Length = Param.VEC_N1N2_SIZE_BYTES); --  for HQC-1

   type Ciphertext_PKE_T is record
      U : U64_Seq_N_Bits_As_Words;
      V : U64_Seq_N_Bits_As_Words;
   end record;

   type Ciphertext_KEM_T is record
      C_PKE : Ciphertext_PKE_T;
      Salt  : Salt_Bytes;
   end record;
   ...
   is
      ...
      --  Puts sequence of U64s to stdout, one byte at a time
      --  little-endian order, but stops after C bytes
      procedure Put (S : in String := "";
                     V : in U64_Seq;
                     C : in N32)
        with Always_Terminates, Global => null;
      ...
      --  Puts sequence of U32s to stdout, one byte at a time
      --  little-endian order, but stops after C bytes
      procedure Put (S : in String := "";
                     V : in U32_Seq;
                     C : in N32)
   ...
   procedure PRNG_Init (PRNG_Ctx               :    out SHAKE.SHAKE256.Context;
     ...
     with Global => null,
          Pre    => Entropy'First                in 0 .. 1 and
                    Entropy'Last                 in 0 .. 48 and
